...
.msgw-panel{
  ...
  color:#e5e7eb; overflow:hidden; z-index:1600;
}
...
(()=>{ // IIFE per isolamento
  ...
  const root = $('#msgw-user'); if (!root) return;
  ...
  async function countUnread(){
    try{
      const j = await fetch(API+'?action=count_unread', {cache:'no-store', credentials:'same-origin'}).then(r=>r.json());
      if (!j || !j.ok) return;
  ...
  async function loadList(){
    ...
    try{
      const j = await fetch(API+'?action=list&limit=50', {cache:'no-store', credentials:'same-origin'}).then(r=>r.json());
      if (!j || !j.ok) throw new Error('list');
      const rows = Array.isArray(j.rows) ? j.rows : [];
      if (!rows.length){ list.innerHTML = `<div class="msgw-empty">Non hai messaggi.</div>`; return; }
      list.innerHTML = '';
      rows.forEach(m=>{
  ...
  list.addEventListener('click', async (e)=>{
    ...
    try{
      ...
      await loadList(); await countUnread();
    }catch(err){ alert('Errore: ' + (err && err.message ? err.message : '')); }
  });
