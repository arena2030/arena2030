...
.team.picked .pick-dot {
  ...
  /* solo le squadre sono cliccabili */
...
evs.forEach(ev=>{
  ...
  d.innerHTML = `
    <div class="team home ${pickedHome?'picked':''} clickable"
         data-team-id="${ev.home_id}" data-team-name="${esc(ev.home_name||('#'+ev.home_id))}"
         data-team-logo="${ev.home_logo?esc(ev.home_logo):''}" role="button" tabindex="0">
    ...
    <div class="team away ${pickedAway?'picked':''} clickable"
         data-team-id="${ev.away_id}" data-team-name="${esc(ev.away_name||('#'+ev.away_id))}"
         data-team-logo="${ev.away_logo?esc(ev.away_logo):''}" role="button" tabindex="0">
      <strong>${ev.away_name||('#'+(ev.away_id||'?'))}</strong>
      ${ev.away_logo? `<img src="${ev.away_logo}" alt="">` : ''}
      <span class="pick-dot"></span>
    </div>
  `;
  // click diretti su Casa/Trasferta
...
// ===== PICK veloce su squadra (clic diretto su Casa/Trasferta) =====
async function doPickQuick(ev, teamId, teamName, teamLogo, cardEl){
  ...
  // guard policy: pick solo round corrente e prima del lock
  try{
  ...
  // aggiornamento ottimistico del puntino sulla card
  ...
  // aggiorna l’icona sulla vita attiva (come facevi già)
  if (lifeElActive){
