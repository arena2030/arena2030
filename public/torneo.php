...
/* === (1) Stato disabilitato per squadre non pickabili con la vita selezionata === */
.team.disabled {
  opacity: .35;
  filter: grayscale(1);
  cursor: not-allowed;
  pointer-events: auto; /* manteniamo il cursore; blocchiamo da JS */
}
...
maybeCheckLockOverlay();
    ...
    if (lives.length){
      lives.forEach((lv,idx)=>{
        ...
        const isLost = ['lost','eliminated','dead','out','persa','eliminata'].includes(lstatus)
                       || (lv.alive===0) || (lv.is_alive===0) || (String(lv.is_alive||'').toLowerCase()==='false');
