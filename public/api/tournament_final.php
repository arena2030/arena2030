...
if ($act === 'finalize') {
  ...
  if (!$res['ok']) { jsonOut($res, 200); } // not_final ecc.
...
if ($act==='leaderboard') {
  ...
  try {
    ...
    if ($has) {
      $winnerIds = array_map('intval',
        $pdo->prepare("SELECT user_id FROM tournament_payouts WHERE tournament_id=? ORDER BY amount DESC")
            ->execute([$tournamentId]) ? $pdo->prepare("") : []
      );
      // fallback semplice in caso di driver che non supporta la doppia esecuzione inline
  ...
  } catch (\Throwable $e) { /* non bloccare la classifica */ }
