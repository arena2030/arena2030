<?php
// /public/api/balance.php — saldo reale dal DB (sessione richiesta)
...
try {
  $st = $pdo->prepare("SELECT COALESCE(coins,0) FROM users WHERE id=? LIMIT 1");
  $st->execute([$uid]);
  $coins = (float)($st->fetchColumn() ?? 0);

  echo json_encode(['ok'=>true,'coins'=>$coins,'formatted'=>number_format($coins,2,'.',''),'ts'=>time()]);
} catch (Throwable $e) {
