...
if (isset($_GET['action'])) {
  ...
  if ($a==='me'){ only_get();
    ...
    $st=$pdo->prepare("SELECT username, COALESCE(coins,0) AS coins FROM users WHERE id=? LIMIT 1");
  ...
  if ($a==='list_prizes'){ only_get();
    ...
    $dir    = $_GET['dir']  ?? 'desc';
    $order = in_array($sort,['name','coins','created'],true)
      ? ($sort==='name'?'p.name':($sort==='coins'?'p.amount_coins':'p.created_at')).' '.(strtolower($dir)==='asc'?'ASC':'DESC')
      : 'p.created_at DESC';
    $w = ['p.is_listed=1']; $p=[];
...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  async function loadMe(){
    const r=await fetch('?action=me'); const j=await r.json();
  ...
  $('#r_prev').addEventListener('click', ()=>{
    const s=$$('.step'); if (s[1].classList.contains('active')){ s[1].classList.remove('active'); s[0].classList.add('active'); $('#r_next').classList.remove('hidden'); $('#r_send').classList.add('hidden'); }
  });
...
const r = await fetch('/api/prize_request.php?action=request',{
  ...
  headers:{ 'Accept':'application/json', 'X-CSRF-Token':'<?= $CSRF ?>' }
    ...
    // ok: chiudi wizard, aggiorna saldo, mostra ok
    closeM('#mdReq');
    await loadMe(); // aggiorna saldo
    openM('#mdOk');
  });

  // close modals
  $$('#mdReq [data-close], #mdReq .modal-backdrop, #mdOk [data-close], #mdOk .modal-backdrop').forEach(x=>x.addEventListener('click', e=>{
    const m = e.target.closest('.modal'); if (m) m.setAttribute('aria-hidden','true'); document.body.classList.remove('modal-open');
  }));
